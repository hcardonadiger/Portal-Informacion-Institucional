...
using Diger.TramitesEstado.Api.Middleware;
using Diger.TramitesEstado.Api.Security;
...
// ── Lo que esta API necesita para funcionar, y nada más ──────────────────────
//
// Antes acá decía `.AddApplication().AddInfrastructure(...)`, y con esas dos líneas entraban en
// el proceso: SMTP, el cliente del agente de IA, ocho repositorios, el cliente de Supabase, un
// servicio de recordatorios en segundo plano, el catálogo de roles y la caché de permisos.
// Ninguno se usaba. Dos de ellos, además, había que registrarlos a mano —con un comentario
// explicando que sin ellos el host ni arrancaba— porque el validador estricto del contenedor
// construye TODOS los manejadores registrados al iniciar, incluidos los de administración de
// permisos que esta API nunca invoca.
//
// Lo que queda es lo que de verdad hace falta: una ventana de solo lectura sobre ocho tablas,
// cuatro clases de consulta y dónde vive SOL.
builder.Services.AddDbContext<ApiDbContext>(opciones =>
    opciones.UseSqlServer(builder.Configuration.GetConnectionString("DefaultConnection")));

builder.Services.Configure<SolOptions>(builder.Configuration.GetSection("Sol"));

builder.Services.AddScoped<ConsultaCatalogo>();
builder.Services.AddScoped<ConsultaFicha>();
builder.Services.AddScoped<ConsultaSincronizacion>();
builder.Services.AddScoped<ConsultaCatalogosAuxiliares>();
...
builder.Services.AddSwaggerGen(opts =>
{
    ...
    opts.OperationFilter<Diger.TramitesEstado.Api.Swagger.RequisitoApiKeyFilter>();
...
// Esta API NO aplica migraciones, ni en desarrollo. El esquema es de PortalDigital y lo mueven
// sus migraciones; que un segundo proceso pudiera tocarlo convertiría en dudoso quién lo cambió.
// Acá solo se lee.
