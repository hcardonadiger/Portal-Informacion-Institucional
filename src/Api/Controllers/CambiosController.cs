...
[Authorize]
[Route("api/v1")]
public sealed class CambiosController(ConsultaSincronizacion sincronizacion) : ControllerBase
{
    ...
    public async Task<ActionResult<CambiosPublicosDto>> Cambios([FromQuery] DateTime? desde, CancellationToken ct)
    {
        ...
        var resultado = await sincronizacion.CambiosAsync(desde.Value, ct);
        return Ok(resultado);
    }
    ...
    {
        var resultado = await sincronizacion.CodigosPublicadosAsync(ct);
        return Ok(resultado);
    }
}
