...
public sealed record ContactoDto(
    int Id, string Nombre, string InstitucionId, string Institucion, string? AreaId, string? Area, string? Cargo,
    ...
    public async Task<IReadOnlyList<ContactoDto>> Handle(GetContactosQuery q, CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q.Buscar))
        {
            ...
            query = query.Where(c =>
                ...
                (c.Cargo != null && c.Cargo.Contains(b)) || (c.Area != null && c.Area.Contains(b)) || c.Institucion.Contains(b));
        }

        return await query
            .OrderBy(c => c.Institucion).ThenBy(c => c.Nombre)
            .Select(c => new ContactoDto(
                c.Id, c.Nombre, c.InstitucionId, c.Institucion, c.AreaId, c.Area, c.Cargo, c.Correo, c.Telefono, c.Notas, c.Origen, c.Activo))
            .ToListAsync(ct);
    }
}
