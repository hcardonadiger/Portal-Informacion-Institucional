    ...
    public static string? Aplicar(Expediente e, ExpedienteInputDto d)
    {
        ...
        foreach (var t in d.Tramites.Where(x => !string.IsNullOrWhiteSpace(x.NombreTramite)))
            e.Agregar(new ExpedienteTramite
            {
                ...
                // Modalidad NO va aqui: mas abajo se asigna normalizada. dev la ponia en crudo en
                // esta linea; conservarla ademas seria asignarla dos veces en el mismo
                // inicializador. Accion si viene de dev y es nueva.
                PlazoLegal = t.PlazoLegal, Tercero = t.Tercero, Accion = t.Accion,
